...
/**
 * OpenTelemetry config + correlation helper for the Worker.
 *
 ...
 * the runtime `instrument()` wiring lives in `index.ts`. Tracing is a no-op when
 * `OTEL_EXPORTER_OTLP_ENDPOINT` is unset — the head sampler drops every span, so
 * nothing is recorded or exported and no network call is made.
 */
...
/** A `ResolveConfigFn` bound to a fixed `service.name`. */
export function traceConfigFor(serviceName: string): ResolveConfigFn<Env> {
  return (env: Env) => buildTraceConfig(env, serviceName);
}
