...
`session-agent.ts` is a single file containing 12 subsystems, 40+ state keys in a stringly-typed key-value table, 15+ raw SQL write paths, and a 57-case runner message handler. Every new feature touches this file, and the lack of internal boundaries means changes in one subsystem routinely break others. The file is too large to hold in context, too coupled to test, and too fragile to modify safely.
...
7. **Colocated tests.** Each extracted class gets a `*.test.ts` file alongside it, exercising the class in isolation against real SQLite (not mocks). This matches the pattern established by `message-store.test.ts` in Phase 1. Tests are part of each phase's definition of done.

## Phases

### Phase 1: MessageStore ✅

**Status**: Complete. Implemented and integrated (696 lines, 1,278 lines of tests). All raw SQL message writes and the `activeTurns` Map have been replaced with `messageStore.*` calls (33 call sites).

**Spec**: `docs/specs/2026-03-20-message-persistence-design.md`

**File**: `durable-objects/message-store.ts`, `durable-objects/message-store.test.ts`
...
The DO implements `RunnerMessageHandlers` — each handler is a short method that calls MessageStore, ChannelRouter, PromptQueue, etc. This eliminates the 57-case switch statement from the DO, replacing it with a typed handler interface.
...
**Storage**: Typed accessors over the existing `state` key-value table. No schema migration, no new storage layer. The `SessionState` class wraps the existing table with typed getters/setters that parse and validate internally (e.g., `get tunnels(): string[]` parses the JSON string, `set status(s)` validates the enum). The stringly-typed `getStateValue`/`setStateValue` methods are deleted from the DO — all access goes through `SessionState`.
...
class SessionLifecycle {
  ...
  /**
   * Run all alarm-driven work. The DO calls this from alarm() and passes
   * each subsystem explicitly — no registration pattern, fully readable
   * top-to-bottom.
   */
  async runAlarmChecks(subsystems: {
    promptQueue: PromptQueue;
    messageStore: MessageStore;
    channelFollowups: { checkDue(): { channelKey: string; flushAt: number }[] };
    metrics: { flushIfNeeded(): Promise<void> };
    interactivePrompts: { expireStale(): number };
  }): Promise<void>;
}
```

The alarm dispatcher calls each subsystem's check method explicitly. The DO wires it in `alarm()`: `this.lifecycle.runAlarmChecks({ promptQueue: this.promptQueue, messageStore: this.messageStore, ... })`. No callback registration — you can read the alarm flow top-to-bottom in one place.
