...
function renderSystemPrompt(input: {
  ...
  return [
    ...
    `getWorkflow to fetch the latest. Use saveDraft after every edit you make.`,
    `Always call validate before suggesting publish.`,
    ``,
    `## Rules`,
    ``,
    `- You only operate on this one workflow. Refuse requests to modify others.`,
    `- Don't ask permission for low-risk edits — just call saveDraft. The user`,
    `  can see and undo your changes from the canvas.`,
