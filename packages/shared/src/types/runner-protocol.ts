/**
 * Runner ↔ DO WebSocket Protocol Types
 *
 * Discriminated unions for all messages exchanged between the Runner process
 * (inside the sandbox) and the SessionAgent Durable Object. Both directions
 * use a `type` discriminant so consumers get properly narrowed types.
 */
...
/** Messages sent from the SessionAgent DO to the Runner process */
export type DOToRunnerMessage =
  ...
  | {
      type: 'plugin-content';
      pluginContent: {
        // `id` is the persona's DB row id (UUID); the runner uses it to
        // resolve per-step persona overrides on `agent_prompt` workflow steps.
...
/** Extract a specific message variant from RunnerToDOMessage by type */
export type RunnerMessageOf<T extends RunnerToDOMessage['type']> = Extract<
  RunnerToDOMessage,
  { type: T }
>;

/** Extract a specific message variant from DOToRunnerMessage by type */
export type DOMessageOf<T extends DOToRunnerMessage['type']> = Extract<
  DOToRunnerMessage,
  { type: T }
>;
