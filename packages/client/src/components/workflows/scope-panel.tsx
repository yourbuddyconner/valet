...
// Map each scope field type to a Badge variant — picks readable contrast in both light + dark modes.
const TYPE_BADGE_VARIANT: Record<ScopeFieldType, BadgeProps['variant']> = {
  string: 'success',
  number: 'default',
  boolean: 'default',
  array: 'warning',
...
function TypeBadge({ type }: { type: ScopeFieldType }) {
  return (
    <Badge variant={TYPE_BADGE_VARIANT[type]} className="shrink-0 !text-[9px] !px-1 !py-px">
      {type}
    </Badge>
  );
}
...
function FieldNode({
  ...
  return (
    <div>
      <div
        className="group flex items-start gap-1.5 py-1 px-1 rounded bg-accent/5 hover:bg-accent/10 cursor-pointer animate-fade-in"
      ...
      >
        {expandable ? (
          <button
            ...
            className="shrink-0 w-3 text-neutral-400 hover:text-foreground leading-none mt-1"
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5">
            <code className="font-mono text-[11px] text-neutral-700 dark:text-neutral-300 truncate">{name}</code>
            <TypeBadge type={field.type} />
            <Copy
              className="ml-auto w-3 h-3 opacity-0 group-hover:opacity-100 text-neutral-400 shrink-0"
              strokeWidth={1.5}
              aria-hidden="true"
            />
          </div>
...
function Section({
  ...
  return (
    <div>
      <div className="text-[10px] text-neutral-500 dark:text-neutral-400 tracking-wider mb-1">{title}</div>
      <div>
