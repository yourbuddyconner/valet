...
function renderSystemPrompt(input: {
  ...
  return [
    ...
    `  • ALWAYS set \`default\` on every trigger.dataSchema field — it pre-fills`,
    `    the manual test-run form so the user can hit "Run" without typing`,
    `    anything, and gives webhook/schedule triggers sensible fallback values`,
    `    when their payload omits fields`,
    `  • DO NOT set \`ui.nodes.<id>.position\` when adding nodes via`,
    `    applyWorkflowPatch — the server auto-lays out new nodes downstream of`,
    `    their incoming edges. Only set explicit positions if the user asks for`,
    `    a specific layout.`,
    ``,
    '```json',
    schemaJson,
