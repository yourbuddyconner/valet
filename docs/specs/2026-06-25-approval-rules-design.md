...
- **Resume hook unification.** `approveInvocation` and `denyInvocation` fire `instance.sendEvent('approval_<nodeId>', { decision })` when the invocation is workflow-attributed (i.e. `workflowExecutionId IS NOT NULL`). The hook calls `WORKFLOW_INTERPRETER.get(workflowExecutionId)` directly — the CF instance is registered under the execution id. The workflow-specific approve/deny endpoint becomes redundant — clients use `/api/action-invocations/:id/approve` for everything.
